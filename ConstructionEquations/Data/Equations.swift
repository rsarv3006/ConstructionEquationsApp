...
let equations: [EquationSection] = [
    ...
    EquationSection(title: "Volumetric Cubic Yards", equations: [
        Equation(id: .cubicYard1, title: "$volume = l * w * h$", description: CubicYardDescription, equation: { sides in
            let l = sides[0]
            let w = sides[1]
            let h = sides[2]
            return l * w * h
        }, filters: [], fieldLabels: ["Length", "Width", "Height", "Cubic Yards"]),
        Equation(id: .cubicYard2, title: "$l = volume / (w * h)$", description: CubicYardDescription, equation: { sides in
            let c = sides[0]
            let w = sides[1]
            let h = sides[2]
            return c / (w * h)
        }, filters: [.advancedFunctions], fieldLabels: ["Cubic Yards", "Width", "Height", "Length"]),
        Equation(id: .cubicYard3, title: "$w = volume / (l * h)$", description: CubicYardDescription, equation: { sides in
            let c = sides[0]
            let l = sides[1]
            let h = sides[2]
            return c / (l * h)
        }, filters: [.advancedFunctions], fieldLabels: ["Cubic Yards", "Length", "Height", "Width"]),
        Equation(id: .cubicYard4, title: "$h = volume / (l x w)$", description: CubicYardDescription, equation: { sides in
            let c = sides[0]
            let l = sides[1]
            let w = sides[2]
            return c / (l * w)
    ...
    EquationSection(title: "Arch Calculator", equations: [
        Equation(id: .arch1, title: "$focal = \\sqrt{width^2 - height^2}$", description: ArchDescription, equation: { inputs in
            let w = inputs[0]
            let h = inputs[1]
            return sqrt(w * w - h * h)
        }, filters: [], fieldLabels: ["Arch Width", "Arch Height", "Focal Point"]),
        Equation(id: .arch2, title: "$width = \\sqrt{focal^2 + height^2}$", description: ArchDescription, equation: { inputs in
            let f = inputs[0]
            let h = inputs[1]
            return sqrt(f * f + h * h)
        }, filters: [.advancedFunctions], fieldLabels: ["Focal Point", "Arch Height", "Arch Width"]),
        Equation(id: .arch3, title: "$height = \\sqrt{width^2 - focal^2}$", description: ArchDescription, equation: { inputs in
            let w = inputs[0]
            let f = inputs[1]
            return sqrt(w * w - f * f)
